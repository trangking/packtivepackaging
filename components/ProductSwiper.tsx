...
export function ProductSwiper({ items, detailHref }: ProductSwiperProps) {
        ...
        {items.map((item, index) => (
          <SwiperSlide key={item.id} className="h-auto">
            <motion.article
              ...
              className="group flex h-full min-h-[370px] flex-col overflow-hidden rounded-[1.1rem] border border-neutral-200/70 bg-white shadow-[0_14px_40px_rgba(103,16,18,0.055)] transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1 hover:border-brand-100 hover:shadow-[0_22px_62px_rgba(103,16,18,0.10)] sm:min-h-[420px] sm:rounded-[1.25rem]"
            >
              ...
              <div className="flex flex-1 flex-col items-start p-5 sm:p-6">
                ...
                <h3 className="mt-3 min-h-[3rem] text-xl font-black leading-snug text-ink sm:min-h-[3.25rem] sm:text-[1.35rem]">{item.title}</h3>
                {detailHref ? (
                  <Link
                    href={detailHref}
                    className="mt-auto inline-flex w-fit rounded-full bg-brand-600 px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-brand-600/20 transition-all duration-300 hover:-translate-y-0.5 hover:bg-brand-700"
                  >
                    ดูข้อมูลหมวดนี้
                  </Link>
                ) : null}
